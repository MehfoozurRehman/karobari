...
function effectivePrice(item: Doc<'catalogItems'>): number {
  return item.discountPct ? Math.round(item.pricePaisa * (1 - item.discountPct / 100)) : item.pricePaisa;
}
...
export const catalogText = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    for (const item of items) {
      ...
      const catName = categories.find((c) => c._id === item.categoryId)?.name ?? 'Other';
      const line =
        `- ${item.name}: ${rs(effectivePrice(item))}` + (item.discountPct ? ` (${item.discountPct}% off, was ${rs(item.pricePaisa)})` : '') + (item.description ? ` — ${item.description}` : '');
...
export const placeOrder = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const line of args.items) {
      ...
      const match =
        catalog.find((i) => i.available && i.name.toLowerCase() === wanted) ?? catalog.find((i) => i.available && (i.name.toLowerCase().includes(wanted) || wanted.includes(i.name.toLowerCase())));
      if (match) resolved.push({ catalogItemId: match._id, qty: line.qty });
      else unresolved.push(line.itemName);
    }
    if (resolved.length === 0) {
      throw new Error(`No items matched the catalog: ${unresolved.join(', ')}`);
    }
...
export const orderStatusForCustomer = internalQuery({
  ...
  handler: async (ctx, args) => {
    const orders = await ctx.db
      .query('orders')
      .withIndex('by_businessId_and_customerPhone', (q) => q.eq('businessId', args.businessId).eq('customerPhone', args.customerPhone))
      .order('desc')
      .take(5);
    const filtered = args.orderNumber ? orders.filter((o) => o.orderNumber === args.orderNumber) : orders;
    if (filtered.length === 0) return 'No orders found for this customer.';
    return filtered.map((o) => `Order #${o.orderNumber}: status=${o.status}, total=${rs(o.totalPaisa)}, payment=${o.paymentMethod}/${o.paymentStatus}`).join('\n');
  },
});
...
export const salesSummary = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const since = args.range === 'today' ? startOfToday : args.range === 'week' ? startOfToday - 6 * 24 * 60 * 60 * 1000 : startOfToday - 29 * 24 * 60 * 60 * 1000;

    const completed = await ctx.db
      .query('orders')
      .withIndex('by_businessId_and_completedAt', (q) => q.eq('businessId', args.businessId).gte('completedAt', since))
      .take(1000);
    const pending = await ctx.db
      .query('orders')
      .withIndex('by_businessId_and_status', (q) => q.eq('businessId', args.businessId).eq('status', 'pending'))
      .take(100);
...
export const pendingOrdersText = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    for (const status of ['pending', 'confirmed', 'preparing', 'ready', 'delivered'] as const) {
      const orders = await ctx.db
        .query('orders')
        .withIndex('by_businessId_and_status', (q) => q.eq('businessId', args.businessId).eq('status', status))
        .take(20);
      for (const o of orders) {
        active.push(`#${o.orderNumber} [${o.status}] ${rs(o.totalPaisa)} — ${o.customerName ?? o.customerPhone}, ${o.customerAddress ?? 'no address'} (${o.paymentMethod}/${o.paymentStatus})`);
      }
    }
...
export const updateOrderStatusByNumber = internalMutation({
  args: {
    businessId: v.id('businesses'),
    orderNumber: v.number(),
    status: v.union(v.literal('confirmed'), v.literal('preparing'), v.literal('ready'), v.literal('delivered'), v.literal('completed'), v.literal('cancelled')),
  },
  returns: v.string(),
  handler: async (ctx, args) => {
    const business = await ctx.db.get('businesses', args.businessId);
...
export const setItemAvailability = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const match = items.find((i) => i.name.toLowerCase() === wanted) ?? items.find((i) => i.name.toLowerCase().includes(wanted));
...
export const billingStatus = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const entry = await ctx.db
      .query('ledgerEntries')
      .withIndex('by_businessId_and_period', (q) => q.eq('businessId', args.businessId).eq('period', period))
      .unique();
    const dueEntries = await ctx.db
      .query('ledgerEntries')
      .withIndex('by_businessId_and_period', (q) => q.eq('businessId', args.businessId))
      .take(24);
    const outstanding = dueEntries.filter((e) => e.status === 'due' || e.status === 'proof_submitted');
