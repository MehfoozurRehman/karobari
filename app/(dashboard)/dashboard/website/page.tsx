...
export default function WebsitePage() {
  ...
  const business = useQuery(api.businesses.getMine, isAuthenticated ? {} : 'skip');
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-stone-900">Website</h1>
        <p className="text-sm text-stone-500">Theme, content, aur aap ki site ka address.</p>
      </div>
      ...
      {business && (
        <div className="flex items-center justify-between rounded-2xl border border-stone-200 bg-white p-5">
          <div>
            ...
            <a href={`https://${business.slug}.${rootDomain}`} target="_blank" rel="noopener noreferrer" className="font-bold text-emerald-700 hover:underline">
              {business.slug}.{rootDomain}
            </a>
          </div>
          <a href={`https://${business.slug}.${rootDomain}`} target="_blank" rel="noopener noreferrer">
          ...
          <div className="grid grid-cols-3 gap-3">
            {THEMES.map((t) => (
              <button
                ...
                className={`rounded-xl border-2 p-3 text-left text-sm transition-colors ${business?.themeId === t.id ? 'border-emerald-600 bg-emerald-50' : 'border-stone-200 hover:border-stone-300'}`}
              >
        ...
        <CardContent className="space-y-4 p-5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" className="rounded-full border-emerald-200 text-emerald-700" disabled={generating} onClick={regenerate}>
          ...
          {content ? (
            <div className="space-y-4">
              ...
              <Button onClick={save} className="w-full rounded-full bg-emerald-700 text-white hover:bg-emerald-800">
                Save Content
              </Button>
            </div>
          ) : (
            <p className="rounded-xl bg-stone-50 p-4 text-sm text-stone-500">
              Abhi default content use ho raha hai. &quot;Generate with AI&quot; par click karein — aap ke business ki details se professional content ban jayega.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="rounded-2xl border-stone-200">
        <CardContent className="flex items-center justify-between p-5">
          <div>
            ...
            <p className="text-xs text-stone-500">AI se banner image generate karein (Gemini).</p>
          </div>
          <Button
            variant="outline"
...
function CustomDomainSection() {
  ...
  return (
    <div className="space-y-3">
      {domains?.map((d) => (
        <div key={d._id} className="space-y-2 rounded-xl border border-stone-200 p-3 text-sm">
          <div className="flex items-center justify-between">
            <p className="font-semibold text-stone-900">{d.domain}</p>
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold capitalize ${d.status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
              {d.status.replace('_', ' ')}
            </span>
          </div>
          {d.status !== 'active' && (
            <p className="rounded-lg bg-stone-50 p-2 text-xs text-stone-600">
              Apne domain registrar mein yeh DNS record add karein: <code className="font-mono">CNAME → cname.vercel-dns.com</code> (ya apex domain ke liye A record → 76.76.21.21). Verify hone par
      ...
      <div className="flex gap-2">
        <Input value={domain} onChange={(e) => setDomain(e.target.value)} placeholder="yourshop.pk" />
        <Button
          variant="outline"
