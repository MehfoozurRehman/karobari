...
const siteContentSchema = z.object({
  ...
  highlights: z.array(z.object({ title: z.string(), text: z.string() })).length(3),
...
export const generate = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!businessId) {
      const me: { business: { _id: Id<'businesses'> } | null } | null = await ctx.runQuery(internal.siteContentAI.ownerBusinessInternal, {});
...
export async function generateSiteContentForBusiness(input: { name: string; category: string; description: string; city?: string }) {
