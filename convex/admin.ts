...
export const overview = query({
  ...
  handler: async (ctx) => {
    ...
    const recentOrders = await ctx.db.query('orders').order('desc').take(1000);
    const todayOrders = recentOrders.filter((o) => o._creationTime >= startOfDay);
    const period = currentPeriod();
    const entries = await ctx.db
      .query('ledgerEntries')
      .withIndex('by_period', (q) => q.eq('period', period))
      .take(1000);
    const accrued = entries.reduce((s, e) => s + e.baseFeePaisa + e.commissionPaisa, 0);
    const allEntries = await ctx.db.query('ledgerEntries').take(2000);
    const collected = allEntries.filter((e) => e.status === 'paid').reduce((s, e) => s + e.baseFeePaisa + e.commissionPaisa, 0);
    const outstanding = allEntries.filter((e) => e.status === 'due' || e.status === 'proof_submitted').reduce((s, e) => s + e.baseFeePaisa + e.commissionPaisa, 0);
    ...
    if (args.decision === 'approved') {
      ...
      if (business && business.status === 'suspended') {
        const otherDue = await ctx.db
          .query('ledgerEntries')
          .withIndex('by_businessId_and_period', (q) => q.eq('businessId', payment.businessId))
          .take(24);
        const stillOwing = otherDue.some((e) => e._id !== payment.ledgerEntryId && (e.status === 'due' || e.status === 'proof_submitted'));
        if (!stillOwing) {
