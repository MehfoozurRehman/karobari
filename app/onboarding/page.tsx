  ...
  async function finish() {
    ...
    try {
      await create({
        ...
        paymentSettings: {
          codEnabled: cod,
          easypaisa: epName && epNumber ? { accountName: epName, number: epNumber } : undefined,
          jazzcash: jcName && jcNumber ? { accountName: jcName, number: jcNumber } : undefined,
        },
      });
        ...
        <CardContent className="space-y-6 p-6 sm:p-8">
          <div>
            <div className="mb-3 flex gap-2">
              {steps.map((s, i) => (
                <div key={s} className={`h-1.5 flex-1 rounded-full ${i <= step ? 'bg-emerald-600' : 'bg-stone-200'}`} />
              ))}
            </div>
          ...
          {step === 0 && (
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Khyber Shinwari" />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Asli Shinwari khaana — tikka, karahi, pulao. Family hall available." />
              </div>
              <div className="space-y-2">
                <Label>Your WhatsApp Number (owner)</Label>
                <Input value={ownerPhone} onChange={(e) => setOwnerPhone(e.target.value)} placeholder="03xx xxxxxxx" />
                <p className="text-[11px] text-stone-400">Is number par aap ko owner agent milega — sales summaries, orders, sab WhatsApp par.</p>
              </div>
              <Button
            ...
            <div className="space-y-4">
              <label className="flex items-center justify-between rounded-xl border border-stone-200 p-4">
                <div>
                  ...
                  <p className="text-xs text-stone-500">Order deliver hone par cash</p>
                </div>
                <input type="checkbox" checked={cod} onChange={(e) => setCod(e.target.checked)} className="h-5 w-5 accent-emerald-700" />
              </label>
              <div className="space-y-3 rounded-xl border border-stone-200 p-4">
                <p className="font-bold text-stone-900">EasyPaisa (optional)</p>
                <div className="grid grid-cols-2 gap-3">
                  <Input value={epName} onChange={(e) => setEpName(e.target.value)} placeholder="Account title" />
                  <Input value={epNumber} onChange={(e) => setEpNumber(e.target.value)} placeholder="03xx xxxxxxx" />
                </div>
              </div>
              <div className="space-y-3 rounded-xl border border-stone-200 p-4">
                <p className="font-bold text-stone-900">JazzCash (optional)</p>
                <div className="grid grid-cols-2 gap-3">
                  <Input value={jcName} onChange={(e) => setJcName(e.target.value)} placeholder="Account title" />
                  <Input value={jcNumber} onChange={(e) => setJcNumber(e.target.value)} placeholder="03xx xxxxxxx" />
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <Button className="flex-1 rounded-full bg-emerald-700 text-white hover:bg-emerald-800" onClick={() => setStep(2)}>
                  Continue
                </Button>
              </div>
            </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="grid gap-3">
                {THEMES.map((t) => (
                  <button
                    ...
                    className={`rounded-xl border-2 p-4 text-left transition-colors ${themeId === t.id ? 'border-emerald-600 bg-emerald-50' : 'border-stone-200 hover:border-stone-300'}`}
                  >
              ...
              <div className="flex gap-3">
                ...
                <Button disabled={busy} className="flex-1 rounded-full bg-emerald-700 text-white hover:bg-emerald-800" onClick={finish}>
