...
export async function sendText(
  ...
  const conversation: Doc<'conversations'> | null = await ctx.runQuery(internal.whatsapp.messagesDb.getConversation, { conversationId: args.conversationId });
  if (!conversation || Date.now() - conversation.lastInboundAt > WINDOW_MS) {
    console.warn(`Skipped WhatsApp send to ${args.to}: 24h window closed`);
    return false;
  }
...
export async function resolveChannel(ctx: ActionCtx, phoneNumberId: string): Promise<{ accessToken: string; businessId: Id<'businesses'> | null } | null> {
