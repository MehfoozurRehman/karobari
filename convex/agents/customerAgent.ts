...
export const run = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const tools = {
      get_catalog: tool({
        description: 'Get the full menu/product list with current prices and discounts.',
      ...
      get_business_info: tool({
        description: 'Get business info: address, city, opening hours, delivery info, payment account numbers.',
        inputSchema: z.object({}),
        execute: async (): Promise<string> => {
      ...
      place_order: tool({
        description: 'Place an order once the customer has confirmed items, delivery address, and payment method. ' + 'Only call after explicit confirmation from the customer.',
        inputSchema: z.object({
          items: z.array(
            z.object({
      ...
      get_order_status: tool({
        description: "Check the status of the customer's recent orders, optionally by order number.",
        inputSchema: z.object({
