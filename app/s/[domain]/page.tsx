...
export default async function StoreHomePage({ params }: { params: Promise<{ domain: string }> }) {
  const { domain } = await params;
  const data = await getStore(decodeURIComponent(domain));
  if (!data) return <StoreUnavailable />;
  if (data.business.status === 'suspended') return <StoreUnavailable name={data.business.name} />;

  const content = (data.siteContent as SiteContent | null) ?? defaultSiteContent(data.business.name, data.business.description);
