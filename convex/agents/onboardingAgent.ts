...
function detectCategory(text: string): Draft['category'] {
  ...
  if (/(restaurant|khaana|food|hotel\s?restaurant|cafe|dhaba|bbq|biryani)/.test(t)) return 'restaurant';
  if (/(hotel|guest\s?house|rooms?)/.test(t)) return 'hotel';
  if (/(salon|barber|hair|parlou?r|beauty)/.test(t)) return 'salon';
  if (/(shop|store|dukaan|dukan|mart|kiryana|garments|mobile)/.test(t)) return 'shop';
  return 'other';
}

function startIntent(text: string): boolean {
  return /(start|shuru|register|business|dukaan|dukan|store|website)/i.test(text);
}
...
export const run = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const existingBusiness: Doc<'businesses'> | null = await ctx.runQuery(internal.agents.onboardingDb.businessByOwnerPhone, { peerPhone: args.peerPhone });
    const session: Doc<'onboardingSessions'> | null = await ctx.runQuery(internal.agents.onboardingDb.getSession, { peerPhone: args.peerPhone });
    ...
    switch (session.step) {
      ...
      case 'catalog': {
        ...
        const itemCount = parsed.categories.reduce((s, c) => s + c.items.length, 0);
        if (itemCount === 0) {
          await reply('Items samajh nahi aaye 😅 Har item ke saath price zaroor likhein, misal:\n\nShinwari Tikka 450\nPulao 400');
          return null;
        }
      ...
      case 'payments': {
        if (!/^skip$/i.test(text)) {
          const match = text.match(/(easypaisa|jazzcash)\s+(\+?\d[\d\s-]{9,14})\s+(.{2,40})/i);
          if (match) {
            const wallet = {
            ...
            await reply('Format samajh nahi aaya. Aise likhein:\n*"easypaisa 03001234567 Muhammad Ali"*\n\nYa *"skip"* likhein.');
            return null;
          }
        }
        await saveStep(ctx, args.peerPhone, 'confirm', draft);
        const itemCount = draft.categories?.reduce((s, c) => s + c.items.length, 0) ?? 0;
        await reply(
      ...
      case 'confirm': {
        if (!/(confirm|haan|yes|theek|ok)/i.test(text)) {
          await reply('Koi masla nahi! Kuch badalna ho to bata dein, ya *"Confirm"* likh kar store live karein.');
          return null;
        }
        ...
        const result: { businessId: Id<'businesses'>; slug: string } = await ctx.runMutation(internal.agents.onboardingDb.registerBusiness, {
          peerPhone: args.peerPhone,
          name: draft.name,
        ...
        await reply(
          `Mubarak ho! 🎉 "${draft.name}" ab LIVE hai!\n\n` +
            `🌐 Store Website: https://${result.slug}.${ROOT_DOMAIN()}\n` +
            `📊 Web Dashboard: https://${ROOT_DOMAIN()}/dashboard\n\n` +
            'Aap is number se apna business operate kar sakte hain:\n' +
            '• "Aaj kitni sales huin?"\n' +
            '• "Active orders dikhao"\n' +
            '• "Status update order #1"\n\n' +
            'Customers ke liye: Dashboard se apna business WhatsApp number connect karein taake aap ke customers aap ke number par order de sakein. 🤖\n\n' +
      ...
      case 'done':
      default: {
        await reply(`Aap ka store pehle se live hai! Dashboard: https://${ROOT_DOMAIN()}/dashboard\nKoi sawal ho to poochein.`);
        return null;
      }
    }
  },
...
async function saveStep(ctx: ActionCtx, peerPhone: string, step: 'name' | 'category' | 'description' | 'catalog' | 'payments' | 'confirm' | 'done', draft: Draft) {
  await ctx.runMutation(internal.agents.onboardingDb.upsertSession, {
    peerPhone,
    step,
    draft,
