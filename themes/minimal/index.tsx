    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="border-b border-slate-100">
        <div className="mx-auto grid max-w-5xl items-center gap-10 px-4 py-16 sm:grid-cols-2 sm:py-24">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-bold leading-tight tracking-tight sm:text-5xl">{content.hero.headline}</h1>
            <p className="mt-4 leading-relaxed text-slate-500">{content.hero.subheadline}</p>
            <Link href="/menu" className="mt-8 inline-block border-b-2 border-slate-900 pb-0.5 text-base font-semibold hover:opacity-70">
              {content.hero.ctaText} →
            </Link>
          </div>
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-slate-100">
            {heroImageUrl ? (
              <Image src={heroImageUrl} alt={business.name} fill className="object-cover" priority />
            ) : (
              <div className="flex h-full items-center justify-center text-6xl">{business.name.charAt(0)}</div>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-slate-100 py-16">
        <div className="mx-auto max-w-xl px-4 text-center">
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-500">{content.about.body}</p>
