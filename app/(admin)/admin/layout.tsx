...
function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading || me === undefined) {
    return <div className="flex min-h-screen items-center justify-center bg-stone-950 text-stone-400">Loading...</div>;
  }
      ...
      <header className="sticky top-0 z-40 border-b border-stone-800 bg-stone-950/90 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
              ...
              {nav.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={cn('rounded-full px-3 py-1.5 text-xs font-semibold', active ? 'bg-emerald-500/15 text-emerald-400' : 'text-stone-400 hover:bg-stone-900 hover:text-stone-200')}
                  >
                    {item.label}
                  </Link>
                );
          ...
          <div className="flex items-center gap-3">
            <Link href="/dashboard" className="text-xs text-stone-400 hover:text-stone-200">
              Owner Dashboard
            </Link>
            <UserButton />
          </div>
