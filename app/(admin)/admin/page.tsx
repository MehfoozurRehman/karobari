...
export default function AdminOverview() {
  ...
  if (!stats) return <div className="h-64 animate-pulse rounded-2xl bg-stone-900" />;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {cards.map((c) => (
          <div key={c.label} className="rounded-2xl border border-stone-800 bg-stone-900/50 p-5">
      ...
      <div className="flex flex-wrap gap-4">
        {stats.pendingProofCount > 0 && (
          <Link href="/admin/billing" className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-5 py-4 text-sm font-semibold text-amber-300 hover:bg-amber-500/20">
            💰 {stats.pendingProofCount} payment proof
            {stats.pendingProofCount > 1 ? 's' : ''} awaiting review →
          </Link>
        )}
        {stats.newQueryCount > 0 && (
          <Link href="/admin/contact" className="rounded-2xl border border-blue-500/30 bg-blue-500/10 px-5 py-4 text-sm font-semibold text-blue-300 hover:bg-blue-500/20">
