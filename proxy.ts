...
export default clerkMiddleware(async (auth, req) => {
  ...
  const isPlatformHost = host === ROOT_DOMAIN || host === `www.${ROOT_DOMAIN}` || host === 'localhost' || host === '127.0.0.1' || host.endsWith('.vercel.app') || host.endsWith('.localhost');
  ...
  if (!isPlatformHost || (host.endsWith('.localhost') && host !== 'localhost')) {
    ...
    if (tenant && tenant !== 'www') {
      return NextResponse.rewrite(new URL(`/s/${tenant}${url.pathname}${url.search}`, req.url));
    }
  }
