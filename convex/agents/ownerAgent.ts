...
export const run = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const tools = {
      sales_summary: tool({
        description: 'Get sales summary: revenue, completed orders, tips, top items for today, this week, or this month.',
      ...
      list_active_orders: tool({
        description: 'List all active (not completed/cancelled) orders with status, customer, and payment info.',
        inputSchema: z.object({}),
      ...
      update_order_status: tool({
        ...
        execute: async (input): Promise<string> => {
          return await ctx.runMutation(internal.agents.ops.updateOrderStatusByNumber, {
            businessId: args.businessId,
      ...
      set_item_availability: tool({
        description: 'Mark a catalog item as available or unavailable (out of stock).',
        inputSchema: z.object({
          itemName: z.string(),
          available: z.boolean(),
        }),
      ...
      billing_status: tool({
        description: "Get Karobari platform billing status: free orders left, this month's fees, unpaid invoices.",
        inputSchema: z.object({}),
        execute: async (): Promise<string> => {
