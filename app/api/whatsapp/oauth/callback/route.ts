...
export async function GET(request: Request) {
  ...
  const dash = (params: string) => Response.redirect(`${origin}/dashboard/whatsapp?${params}`);
  ...
  try {
    ...
    if (!appId || !appSecret || !convexUrl) {
      throw new Error('Missing configuration: META_APP_ID, META_APP_SECRET, or CONVEX_URL');
    }

    const redirectUri = `${origin}/api/whatsapp/oauth/callback`;

    const tokenRes = await fetch(`${GRAPH_BASE}/oauth/access_token?client_id=${appId}&client_secret=${appSecret}&code=${encodeURIComponent(code)}&redirect_uri=${encodeURIComponent(redirectUri)}`);
    ...
    const wabaRes = await fetch(`${GRAPH_BASE}/${facebookUserId}/owned_whatsapp_business_accounts`, { headers: { Authorization: `Bearer ${accessToken}` } });
    ...
    const phoneRes = await fetch(`${GRAPH_BASE}/${wabaId}/phone_numbers?limit=1`, { headers: { Authorization: `Bearer ${accessToken}` } });
    ...
    const infoRes = await fetch(`${GRAPH_BASE}/${phoneNumberId}?fields=display_phone_number`, { headers: { Authorization: `Bearer ${accessToken}` } });
