...
export const ensureUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const adminEmails = (process.env.ADMIN_EMAILS ?? '')
      ...
      .filter(Boolean);
    const role = adminEmails.includes(args.email.toLowerCase()) ? ('admin' as const) : ('owner' as const);
...
export const getMe = query({
  ...
  handler: async (ctx) => {
    ...
    if (!user) return null;
    const business = user.businessId ? await ctx.db.get('businesses', user.businessId) : null;
