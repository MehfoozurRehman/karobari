...
export const businessCategory = v.union(v.literal('restaurant'), v.literal('hotel'), v.literal('salon'), v.literal('shop'), v.literal('other'));

export const businessStatus = v.union(v.literal('onboarding'), v.literal('active'), v.literal('grace'), v.literal('suspended'));

export const orderStatus = v.union(v.literal('pending'), v.literal('confirmed'), v.literal('preparing'), v.literal('ready'), v.literal('delivered'), v.literal('completed'), v.literal('cancelled'));

export const paymentSettings = v.object({
  codEnabled: v.boolean(),
  easypaisa: v.optional(v.object({ accountName: v.string(), number: v.string() })),
  jazzcash: v.optional(v.object({ accountName: v.string(), number: v.string() })),
  bank: v.optional(
    v.object({
      bankName: v.string(),
...
export default defineSchema({
  ...
  catalogItems: defineTable({
    ...
    imageStatus: v.union(v.literal('none'), v.literal('generating'), v.literal('ready')),
    available: v.boolean(),
    sortOrder: v.number(),
  })
    .index('by_businessId', ['businessId'])
  ...
  orders: defineTable({
    ...
    paymentStatus: v.union(v.literal('unpaid'), v.literal('proof_submitted'), v.literal('paid')),
  ...
  onboardingSessions: defineTable({
    peerPhone: v.string(),
    step: v.union(v.literal('name'), v.literal('category'), v.literal('description'), v.literal('catalog'), v.literal('payments'), v.literal('confirm'), v.literal('done')),
    draft: v.any(),
    businessId: v.optional(v.id('businesses')),
  ...
  ledgerEntries: defineTable({
    ...
    status: v.union(v.literal('open'), v.literal('due'), v.literal('proof_submitted'), v.literal('paid'), v.literal('waived')),
  ...
  platformPayments: defineTable({
    ...
    method: v.union(v.literal('easypaisa'), v.literal('jazzcash'), v.literal('bank'), v.literal('manual')),
    tid: v.optional(v.string()),
    screenshotStorageId: v.optional(v.id('_storage')),
  ...
  salaryEntries: defineTable({
    ...
    period: v.string(),
    amountPaisa: v.number(),
    type: v.union(v.literal('salary'), v.literal('advance'), v.literal('bonus'), v.literal('deduction')),
    note: v.optional(v.string()),
    paidAt: v.number(),
  })
    .index('by_businessId_and_period', ['businessId', 'period'])
  ...
  customDomains: defineTable({
    businessId: v.id('businesses'),
    domain: v.string(),
    status: v.union(v.literal('pending_dns'), v.literal('verifying'), v.literal('active'), v.literal('error')),
