...
export const renameCategory = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!category || category.businessId !== business._id) throw new Error('Category not found');
...
export const deleteCategory = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const items = await ctx.db
      .query('catalogItems')
      .withIndex('by_businessId_and_categoryId', (q) => q.eq('businessId', business._id).eq('categoryId', args.categoryId))
      .take(500);
...
export const updateItem = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!item || item.businessId !== business._id) throw new Error('Item not found');
    ...
    if (args.discountPct !== undefined) patch.discountPct = args.discountPct === null ? undefined : args.discountPct;
    if (args.categoryId !== undefined) patch.categoryId = args.categoryId === null ? undefined : args.categoryId;
...
async function bulkImportForBusiness(
  ...
  for (const category of categories) {
    let categoryId: Id<'catalogCategories'> | undefined = existingCategories.find((c) => c.name.toLowerCase() === category.name.toLowerCase())?._id;
