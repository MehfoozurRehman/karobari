...
export const pendingInternal = internalQuery({
  args: {},
  handler: async (ctx) => {
    const pending = await ctx.db.query('customDomains').withIndex('by_domain').take(200);
...
export const add = action({
  ...
  returns: v.null(),
  handler: async (ctx, args) => {
    const businessId = await ctx.runQuery(internal.domains.ownerBusinessIdInternal, {});
...
export const checkPending = internalAction({
  ...
  handler: async (ctx) => {
    ...
    for (const d of pending) {
      try {
        const res = await fetch(vercelBase(`/v6/domains/${d.domain}/config`), { headers: vercelHeaders() });
        const json = await res.json();
