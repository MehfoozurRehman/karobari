...
The OAuth flow works today only for **app admins / developers / testers**. To let _any_ business authorize their number, the app needs **Advanced Access** to `whatsapp_business_messaging` + `whatsapp_business_management` via App Review (business verification — a prerequisite — is already done). This does **not** require Tech Provider / Solution Partner status.
