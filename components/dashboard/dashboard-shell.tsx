...
import { LayoutDashboard, ShoppingBag, UtensilsCrossed, Globe, MessageSquare, Wallet, Users, Star, Receipt, Settings } from 'lucide-react';
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const needsOnboarding = me !== undefined && me !== null && me.business === null;
    ...
    <div className="flex min-h-screen bg-[#faf9f6]">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 flex-col border-r border-stone-200 bg-white lg:flex">
        <div className="flex h-16 items-center gap-2.5 border-b border-stone-200 px-5">
          <Image src="/karobari_logo.webp" alt="Karobari" width={30} height={30} className="rounded-lg object-contain" />
        ...
        {me?.business && (
          <div className="border-t border-stone-200 p-4">
            <p className="truncate text-sm font-bold text-stone-900">{me.business.name}</p>
            <a
            ...
            >
              {me.business.slug}.{process.env.NEXT_PUBLIC_ROOT_DOMAIN ?? 'karobari.shop'}
            </a>
          </div>
        )}
      </aside>
      ...
      <div className="flex min-h-screen flex-1 flex-col lg:pl-60">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-stone-200 bg-white/85 px-4 backdrop-blur-xl sm:px-6">
          <div className="flex items-center gap-3 lg:hidden">
            <Image src="/karobari_logo.webp" alt="Karobari" width={28} height={28} className="rounded-lg object-contain" />
            <span className="font-extrabold text-stone-900">Karobari.</span>
          </div>
          <div className="hidden lg:block" />
          <div className="flex items-center gap-3">
            {me?.user.role === 'admin' && (
              <Link href="/admin" className="rounded-full border border-stone-200 px-3 py-1 text-xs font-semibold text-stone-600 hover:bg-stone-100">
                Admin Panel
              </Link>
            )}
            <UserButton />
        ...
        {suspended && (
          <div className="border-b border-red-200 bg-red-50 px-6 py-3 text-sm font-medium text-red-800">
            Your account is suspended due to an unpaid invoice. Your storefront and WhatsApp agent are paused.{' '}
        ...
        <nav className="sticky bottom-0 z-30 flex justify-around border-t border-stone-200 bg-white py-2 lg:hidden">
          {nav.slice(0, 5).map((item) => {
            const active = item.href === '/dashboard' ? pathname === '/dashboard' : pathname.startsWith(item.href);
            return (
              <Link key={item.href} href={item.href} className={cn('flex flex-col items-center gap-0.5 rounded-lg px-3 py-1 text-[10px] font-medium', active ? 'text-emerald-700' : 'text-stone-500')}>
