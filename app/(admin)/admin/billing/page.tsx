...
export default function AdminBilling() {
  ...
  const queue = useQuery(api.admin.paymentReviewQueue, isAuthenticated ? {} : 'skip');
  const ledger = useQuery(api.admin.ledgerOverview, isAuthenticated ? {} : 'skip');
  ...
  return (
    <div className="space-y-10">
      <section className="space-y-4">
        ...
        {queue?.length === 0 && <p className="rounded-2xl border border-stone-800 py-10 text-center text-sm text-stone-500">No payment proofs awaiting review.</p>}
        <div className="grid gap-4 md:grid-cols-2">
          {queue?.map((p) => (
            <div key={p._id} className="space-y-3 rounded-2xl border border-stone-800 bg-stone-900/50 p-5">
              <div className="flex items-center justify-between">
                ...
                <p className="text-lg font-extrabold text-emerald-400">{formatPaisa(p.amountPaisa)}</p>
              </div>
              {p.screenshotUrl && (
                <a href={p.screenshotUrl} target="_blank" rel="noopener noreferrer">
                  <Image src={p.screenshotUrl} alt="Payment proof" width={400} height={240} className="max-h-60 w-full rounded-xl border border-stone-800 object-contain" />
                </a>
              )}
      ...
      <section className="space-y-4">
        <h2 className="text-lg font-extrabold">Ledger — {ledger?.period}</h2>
        <LedgerTable
        ...
        {ledger && ledger.overdueOther.length > 0 && (
          <>
            <h2 className="pt-4 text-lg font-extrabold text-amber-400">Overdue from earlier months</h2>
            <LedgerTable
...
function LedgerTable({ entries, onManual, onWaive }: { entries: LedgerRow[]; onManual: (id: string) => void; onWaive: (id: string) => void }) {
  if (entries.length === 0) {
    return <p className="rounded-2xl border border-stone-800 py-10 text-center text-sm text-stone-500">No ledger entries.</p>;
  }
  return (
          ...
          {entries.map((e) => (
            <tr key={e._id} className="border-b border-stone-900">
              ...
              <td className="px-4 py-3">
                <span className={`rounded-full px-2.5 py-1 text-xs font-bold capitalize ${STATUS_STYLES[e.status]}`}>{e.status.replace('_', ' ')}</span>
              </td>
